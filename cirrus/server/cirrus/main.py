...
from .experiment_recipes import RemoteSettings, client
...
@app.get("/")
def read_root():
    return {"Hello": "World", "client": client, "r": remote_setting.get_recipes()}
...
async def compute_features():
    ...
    res = client.handle_enrollment(
        json.dumps({"clientId": "test", "requestContext": {}})
    )
    ts = datetime.now()
    print(ts - t)
...
# force to run on start up
# set experiments should be updated whenever new copy of recipes received
# move .env value
